(() => {
  ...
  async function updateForDate(date){
    ...
    const realPulse=isNativeDaily(state.daily),temp=temperatureSummary(date),s=state.daily?.summary||{},p90=Number(s.discharge_p90_m3s),max=Number(s.discharge_max_m3s),active=Number(s.segments_positive),total=Number(s.segments_with_discharge),zero=Number(s.zero_fraction);
    ...
    updateStatus();$$('#quickDates button').forEach(b=>b.classList.toggle('active',b.dataset.date===token));scheduleRender();
  }
  ...
  function applyLanguage(){
    ...
    if(current&&state.availableDates.length)updateForDate(current);
  }
  ...
  $$('.panel-tab').forEach(b=>b.onclick=()=>setTab(b.dataset.tab));$$('.mobile-nav-item').forEach(b=>b.onclick=()=>setTab(b.dataset.mobileTab));
  ...
  mapEl.addEventListener('pointerdown',e=>{state.dragging=true;mapEl.setPointerCapture(e.pointerId);state.dragStart={x:e.clientX,y:e.clientY};state.centerStart=project(state.center.lon,state.center.lat);});
  mapEl.addEventListener('pointermove',e=>{if(!state.dragging)return;const dx=e.clientX-state.dragStart.x,dy=e.clientY-state.dragStart.y,p=unproject(state.centerStart.x-dx,state.centerStart.y-dy);state.center={lon:p.lon,lat:p.lat};scheduleRender();});
  mapEl.addEventListener('pointerup',()=>state.dragging=false);mapEl.addEventListener('pointercancel',()=>state.dragging=false);mapEl.addEventListener('wheel',e=>{e.preventDefault();zoom(e.deltaY<0?1:-1);},{passive:false});
  ...
  if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=864').catch(()=>{}));let lastMapWidth=0;new ResizeObserver(()=>{const w=mapEl.clientWidth;if(lastMapWidth&&Math.abs(w-lastMapWidth)>48){fitMS();}else{scheduleRender();}lastMapWidth=w;}).observe(mapEl);window.addEventListener('orientationchange',()=>setTimeout(fitMS,220));
