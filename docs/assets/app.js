(() => {
  ...
  async function loadOutflow(){
    ...
    if(outflowHint)outflowHint.textContent=L('Execute PREPARAR_SAIDA_HIDRICA_MS.cmd','Ejecuta PREPARAR_SAIDA_HIDRICA_MS.cmd');
  }
  ...
  function drawOutflowChart(date){
    ...
    if(!rows.length){c.fillStyle='rgba(210,235,233,.5)';c.font='10px Inter, Segoe UI, Arial';c.fillText(L('Série de saída ainda não preparada','Serie de salida aún no preparada'),8,h/2);if(outflowPeakLabel)outflowPeakLabel.textContent=L('pico anual —','pico anual —');return;}
  ...
  function updateOutflow(date){const v=outflowValueFor(isoDate(date));if(outflowValue)outflowValue.textContent=numberText(v);if(outflowSub)outflowSub.textContent=state.outflow?L('soma dos cruzamentos de saída · GloFAS v4','suma de los cruces de salida · GloFAS v4'):L('série agregada ainda não preparada','serie agregada aún no preparada');drawOutflowChart(date);}
  ...
  if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=865').catch(()=>{}));let lastMapWidth=0;new ResizeObserver(()=>{const w=mapEl.clientWidth;if(lastMapWidth&&Math.abs(w-lastMapWidth)>48){fitMS();}else{scheduleRender();}lastMapWidth=w;const d=dateInput.value?parseDate(dateInput.value):null;if(d)drawOutflowChart(d);}).observe(mapEl);if(outflowChartWrap)new ResizeObserver(()=>{const d=dateInput.value?parseDate(dateInput.value):null;if(d)drawOutflowChart(d);}).observe(outflowChartWrap);window.addEventListener('orientationchange',()=>setTimeout(()=>{fitMS();const d=dateInput.value?parseDate(dateInput.value):null;if(d)drawOutflowChart(d);},220));
